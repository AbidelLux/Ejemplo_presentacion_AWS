...
// Modo no interactivo pensado para ser envuelto por un servidor API (HTTP).
// A diferencia de start(), no limpia pantalla ni espera "presionar enter":
// procesa una linea/comando por cada linea leida de stdin y marca el final
// de cada respuesta con el centinela ###END### para que el proceso que lo
// invoca (ver api/server.js) sepa donde termina la salida de cada comando.
void scanner::startApi()
{
    string texto;
    while (getline(cin, texto))
    {
        ...
        {
            errores("SYSTEM", string("Excepcion no controlada: ") + e.what());
        }
        cout << "###END###" << endl;
        cout.flush();
    }
